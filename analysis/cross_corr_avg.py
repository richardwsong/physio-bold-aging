...
def process_participant(sub_id, age):
    ...
    physio_data = loadmat(f'/data1/neurdylab/songrw/derivates/hrv_er/preproc_physio_ses-pre/{sub_id}/{sub_id}_ses-pre_task-rest_physio_physOUT.mat')
    ...
    # Load BOLD data
    bold_img = nib.load(f'/data1/neurdylab/datasets/HRV-ER/HRV-ER_proc/{sub_id}/ses-pre/func/ants_out/{sub_id}_ses-pre_task-rest_bold_mo_EPI2MNI_sm_nr.nii.gz')
    bold_data = bold_img.get_fdata()
    
    # Load Schaefer atlas
    mask_dir = "data/masks/Schaefer2018_100Parcels_17Networks_order_FSLMNI152_2mm.nii" # Schaefer 100 parcel mask for gray matter
    schaefer_img = nib.load(mask_dir)
    schaefer_mask = schaefer_img.get_fdata()
    
    # Extract gray matter timeseries
    gm_timeseries = extract_gray_matter_timeseries(bold_data, schaefer_mask)
