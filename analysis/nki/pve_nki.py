...
def create_design_matrix(df, physio_stats, covariates=None):
    ...
    Parameters:
        ...
        physio_stats (np.ndarray): Array of physiological statistics (lf, hf, avg_hr, sd_rv, br).
        covariates (list, optional): List of covariates to include. Options are:
            ...
            - 'sd_rv': Include standard deviation of respiratory variation
    ...
    covariate_map = {
        'gender': np.array(df['gender'] == 'M', dtype=float)[:, np.newaxis],
        'lf': physio_stats[:, 0:1],
        'hf': physio_stats[:, 1:2],
        'avg_hr': physio_stats[:, 2:3],
        'sd_rv': physio_stats[:, 3:4],
        'br': physio_stats[:, 4:5]
    }
