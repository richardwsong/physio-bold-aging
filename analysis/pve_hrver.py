...
def calc_physio_features(name, session):
    data = loadmat(f"/data1/neurdylab/songrw/derivates/hrv_er/preproc_physio_ses-{session}/{name}/{name}_ses-{session}_task-rest_physio_physOUT.mat") # Path to preprocessed physio data
...
def calc_percent_variance_explained(name, mask_indices, session): 
    ...
    nii_path = f'/data1/neurdylab/datasets/HRV-ER/HRV-ER_proc/{name}/ses-{session}/func/ants_out/{name}_ses-{session}_task-rest_bold_mo_EPI2MNI_sm_nr.nii.gz'
