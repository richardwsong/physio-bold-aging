# Creates Whole Brain Cross Correlation Maps for each lag and group comparison (Figures 4 and 5).
...
def run_randomise(input_4d, output_prefix, design_mat, contrast_mat, n_permutations=5000):
    ...
    cmd = [
        ...
        '-m', '/path/to/MNI152_T1_2mm_brain.nii'
    ]
...
def save_design_contrast_matrices(group1_size, group2_size):
    ...
    np.savetxt("/path/to/output/design.txt", design_mat, fmt="%d")
    np.savetxt("/path/to/output/contrast.txt", contrast_mat, fmt="%d")
...
def process_single_lag(subs_id, ages, lag, start_lag, x, y, z):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=16) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            try:
                hr_corr, rv_corr, co2_corr = future.result()
                if age < 50:
                    hr_young_data.append(hr_corr)
                    rv_young_data.append(rv_corr)
                    co2_young_data.append(co2_corr)
                else:
                    hr_old_data.append(hr_corr)
                    rv_old_data.append(rv_corr)
    ...
    rv_young_4d = np.stack(rv_young_data, axis=3) if rv_young_data else np.zeros((x, y, z, 0))
    rv_old_4d = np.stack(rv_old_data, axis=3) if rv_old_data else np.zeros((x, y, z, 0))
    ...
    return hr_young_4d, hr_old_4d, rv_young_4d, rv_old_4d, co2_young_4d, co2_old_4d
...
def process_participant_single_lag(sub_id, lag, start_lag):
    ...
    """
    data = loadmat(f"/path/to/physio/{sub_id}/{sub_id}_physio.mat")
    bold_path = f"/path/to/BOLD/{sub_id}_bold.nii.gz"

    hr = data['REGS']['hr'][0, 0].flatten()
    rv = data['REGS']['rv'][0, 0].flatten()
    co2 = data['RESP']['co2'][0, 0].flatten()

    bold_img = nib.load(bold_path)
    bold_data = bold_img.get_fdata()
    mask = nib.load("/path/to/MNI152_T1_2mm_brain.nii").get_fdata().astype(bool)
    ...
    rv = detrend(rv - np.mean(rv)) / np.std(rv)
...
def main(start_lag=-2, end_lag=20):
    ...
    """
    fsl_config()
    df = pd.read_csv("/path/to/age_gender.csv")
    subs_id = np.array(df.iloc[:, 0])
    ...
    x, y, z, _ = nib.load(f"/path/to/first/BOLD.nii.gz").shape
    ...
    for lag_idx in range(num_lags):
        ...
        hr_young_4d, hr_old_4d, rv_young_4d, rv_old_4d, co2_young_4d, co2_old_4d = process_single_lag(
        ...
        for measure, young_data, old_data, tstat1_list, tstat2_list, corrp1_list, corrp2_list in [
            ...
            ('rv', rv_young_4d, rv_old_4d, rv_tstat1_list, rv_tstat2_list, rv_corrp1_list, rv_corrp2_list), 
            ...
            run_randomise(
                ...
                os.path.join(output_dir, "contrast.txt"),
                n_permutations=1000
            )
        ...
        del hr_young_4d, hr_old_4d, rv_young_4d, rv_old_4d, co2_young_4d, co2_old_4d, combined_data
    
    # Save final 4D results
    affine = nib.load(f"/path/to/first/BOLD.nii.gz").affine
    for data_list, filename in [
        ...
        (rv_tstat1_list, os.path.join(output_dir, "rv_young-gt-old_tstat_4d.nii.gz")),
        (rv_tstat2_list, os.path.join(output_dir, "rv_old-gt-young_tstat_4d.nii.gz")),
        (rv_corrp1_list, os.path.join(output_dir, "rv_young-gt-old_tfce_corrp_4d.nii.gz")),
        (rv_corrp2_list, os.path.join(output_dir, "rv_old-gt-young_tfce_corrp_4d.nii.gz")), 
